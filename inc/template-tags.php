...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		$time_string = '<span class="fa_icon"><i class="fa fa-calendar"></i></span><time class="entry-date published updated" datetime="%1$s">%2$s</time>';
...
if ( ! function_exists( 'twenty_twenty_one_posted_by' ) ) {
	...
	function twenty_twenty_one_posted_by() {
		if ( ! get_the_author_meta( 'description' ) && post_type_supports( get_post_type(), 'author' ) ) {
			...
			echo '<span class="fa_icon"><i class="fa fa-edit"></i></span>';
			printf(
				/* translators: %s author name. */
				esc_html__( 'By %s', 'Cargo' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'Cargo' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				$categories_list = get_the_category_list( __( ', ', 'Cargo' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: list of categories. */
						'<div class="cat-links"><span class="fa_icon"><i class="fa fa-list-ul"></i></span>' . esc_html__( '%s', 'Cargo' ) . ' </div>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', 'Cargo' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: list of tags. */
						'<div class="tags-links"><span class="fa_icon"><i class="fa fa-tag"></i></span>' . esc_html__( '%s', 'Cargo' ) . '</div>',
	...
	function twenty_twenty_one_the_posts_navigation() {
		...
		the_posts_pagination(
			array(
				...
				'before_page_number' => esc_html__( 'Page ', 'Cargo' ),
				'mid_size'           => 0,
				'prev_text'          => sprintf(
